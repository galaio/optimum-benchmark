...
func HandleResponse(resp *protobuf.Response, counter *int32) {
	...
	case protobuf.ResponseType_MessageTraceGossipSub:
		HandleGossipSubTrace(resp.GetData(), false, nil)
	case protobuf.ResponseType_MessageTraceMumP2P:
		HandleOptimumP2PTrace(resp.GetData(), false, nil)
...
func HandleResponseWithTracking(ip string, resp *protobuf.Response, counter *int32,
	...
	default:
		fmt.Printf("[WARN] unknown gRPC response command=%v (data=%dB)\n",
			resp.GetCommand(), len(resp.GetData()))
	}
}

// HandleGossipSubTrace decodes trace payloads for NODE_MODE=gossipsub.
// getoptimum/p2pnode typically emits optimum.pb TraceEvent (same wire as mump2p) even in
// GossipSub mode; fall back to upstream libp2p pubsub.pb TraceEvent if that fails.
func HandleGossipSubTrace(data []byte, writeTrace bool, traceCh chan<- string) {
	if len(data) == 0 {
		return
	}
	optEvt := &optsub.TraceEvent{}
	errOpt := proto.Unmarshal(data, optEvt)
	if errOpt == nil {
		HandleOptimumP2PTrace(data, writeTrace, traceCh)
		return
	}

	evt := &pubsubpb.TraceEvent{}
	if err := proto.Unmarshal(data, evt); err != nil {
		fmt.Printf("[TRACE] GossipSub decode error: libp2p-pubsub %v; optimum.pb %v raw=%dB head=%s\n",
			err, errOpt, len(data), HeadHex(data, 64))
		return
	}

	typeStr := pubsubpb.TraceEvent_Type_name[int32(evt.GetType())]
	var peerID peer.ID
	if evt.PeerID != nil {
